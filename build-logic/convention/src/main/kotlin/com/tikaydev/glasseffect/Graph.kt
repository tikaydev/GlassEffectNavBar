...
private class Graph(
    ...
    private val ignoredProjects = root.providers
        .gradleProperty("graph.ignoredProjects")
        .map { it.split(",").toSet() }
        .orElse(emptySet())

    private val supportedConfigurations = root.providers
...
internal fun Project.configureGraphTasks() {
    ...
    // New task to overwrite README.md
    tasks.register<GraphOverwriteTask>("graphOverwrite") {
        projectPath = this@configureGraphTasks.path
        input = dumpTask.flatMap { it.output }
        legend = dumpTask.flatMap { it.legend }
        output = this@configureGraphTasks.layout.projectDirectory.file("README.md")
...
@CacheableTask
private abstract class GraphOverwriteTask : DefaultTask() {
    ...
    override fun getDescription() = "Overwrites README.md with the corresponding dependency graph."

    @TaskAction
    operator fun invoke() = with(output.get().asFile) {
        // Always ensure the file exists and has the necessary markers, overwriting if necessary.
        // This guarantees a consistent base format before injecting content.
        val initialContent = """
            # `${projectPath.get()}`
